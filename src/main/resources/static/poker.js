...
(function() {
	...
	if (window.CanvasRenderingContext2D) {
		...
		CanvasRenderingContext2D.prototype.drawNumberCard = function(x, y, size, suit, point) {
			var ax = function(n) {
				return x + n * size / 200;
			}, ay = function(n) {
				return y + n * size / 200;
			...
			this.font = "80px sans-serif";
			if (point.length == 1) {
				this.fillText(point, ax(50), ay(125), 100);
			} else if (point.length == 2) {
				this.fillText(point, ax(25), ay(125), 100);
			} else {
				this.fillText(point, ax(20), ay(125), 100);
			}
		};
		...
		CanvasRenderingContext2D.prototype.drawPokerCard = function(x, y, size, suit, point) {
			...
			if (size >= 100) {
				if (point !== 'o') {
					this.fillPokerSymbol(ax(40), ay(65), as(70), point);
